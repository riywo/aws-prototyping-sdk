 ...
 */
export const buildInvokeOpenApiGeneratorCommand = (options: GenerationOptions): CommandDetails => {
  ...
  const specPath = path.relative(workingDir, path.join(options.outputPath, options.specPath));
...
export const buildCleanGeneratedClientCommand = (clientOutdir: string): CommandDetails => {
  const workingDir = path.resolve(
    __dirname,
    "..",
    "..",
  ...
  const clientPath = path.relative(workingDir, clientOutdir);

  return {
    command: `./clean-generated-client --client-path ${clientPath}`,
    workingDir,
  };
};
...
export interface GenerateCdkInfrastructureOptions {
  ...
  /**
   * Generated client package name for importing the generated client
   */
...
export const buildGenerateCdkInfrastructureCommand = (
  ...
  const specPath = path.relative(workingDir, path.join(options.sourcePath, options.specPath));
  const sourcePath = path.relative(workingDir, options.sourcePath);
  const resourcesPath = options.resourcePath ? path.relative(workingDir, options.resourcePath) : undefined;

  return {
    command: `./generate-type-safe-cdk-construct --spec-path ${specPath} --source-path ${sourcePath} --language ${options.language} --infra-package ${options.infraPackage} --generated-client-package ${options.generatedClientPackage}${resourcesPath ? ` --resource-path ${resourcesPath}` : ''}`,
    workingDir,
  };
}
