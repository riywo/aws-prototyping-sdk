...
export class SmithyDefinition extends Component {
  ...
  constructor(project: TypeSafeApiModelProject, options: SmithyDefinitionOptions) {
    super(project);
...
`,
    ...
    project.generateTask.exec(`./gradlew -p . generate -Pconfig=smithy-build.json -Pdiscover=${modelDir} -Poutput=${options.buildOutputDir}`);
