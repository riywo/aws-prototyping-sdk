...
export const generateJavaTypeSafeCdkConstruct = (options: ConstructGenerationArguments) => {
  // Write the parsed spec to the resources directory, so it can be packaged into the jar
  writeFile(path.join(options.resourcePath, ".api.json"), fs.readFileSync(options.specPath, 'utf-8'), { readonly: true });
  writeFile(path.join(options.sourcePath, "ApiProps.java"), `package ${options.infraPackage};
...
import software.aws.awsprototypingsdk.typesafeapi.OpenApiIntegration;

import ${options.generatedClientPackage}.api.OperationConfig;
...
`, { readonly: true });
  ...
  writeFile(path.join(options.sourcePath, "Api.java"), `package ${options.infraPackage};
...
import ${options.generatedClientPackage}.api.OperationLookup;
