...
import { Language, DocumentationFormat } from "../../../src";
...
describe("OpenAPIGateway Ts Project - Regenerate Clients/Docs Tests", () => {
  ...
  const testLanguage = Language.TYPESCRIPT;
  ...
  it("Maintain References to Generated Client Projects without Regenerating", () => {
    ...
    const project = new OpenApiGatewayTsProject({
      ...
      name,
      generatedCodeDir,
      clientLanguages: [Language.TYPESCRIPT, Language.JAVA],
    });
    project.synth();

    expect(project.generatedClients[Language.JAVA]).toBeDefined();
    expect(project.generatedClients[Language.TYPESCRIPT]).toBeDefined();
    expect(project.generatedClients[Language.PYTHON]).not.toBeDefined();
    ...
    expect(projectOther.generatedClients[Language.JAVA]).toBeDefined();
    expect(
      projectOther.generatedClients[Language.TYPESCRIPT]
    ).toBeDefined();

    // We should not have generated a project for a language we did not request
    expect(
      projectOther.generatedClients[Language.PYTHON]
    ).not.toBeDefined();
  });
});
